...
export default class Auth {
  static signup(req, res, next) {
    ...
    if (!firstName || !validator.isName(firstName)) return sendErrorResponse(res, 422, 'first_name is required, and it should contain only alphabets');
    if (!lastName || !validator.isName(lastName)) return sendErrorResponse(res, 422, 'last_name is required, and it should contain only alphabets');
    ...
    if (!phoneNumber || !validator.isPhoneNumber(phoneNumber)) return sendErrorResponse(res, 422, 'phoneNumber is required, and it should be 11 characters - 08167679018, 07098987634');
